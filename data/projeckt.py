...
class Glaw_form(QWidget):
    def __init__(self):
        ...
        self.rols_royce.clicked.connect(lambda: self.all_form("rols royce"))
                ...
                for i in range(len(z)):
                    try:
                        ...
                        cur.execute("""INSERT INTO mashin("марка", "модель", "коробка передач", "Макс.скорость", "разгон 0 - 100",
                                "Общая масса", "Объём. топ. бака", "картинка", "Длинна", "Ширина", "Высота", "Дор. просвет", "цена",
...
class Play(QWidget):
    ...
    def nachinka(self):
        con1 = sqlite3.connect('rabota.db')
        # Создание курсора
        cur = con1.cursor()
...
class Result_play(QWidget):  ## вывод сообщений результатов
    def __init__(self):
        ...
        global count_for_play
        if true_otw < 4:
            self.medal.setIcon(QIcon('bronz.jpg'))
            ...
            self.medal.setIcon(QIcon('bronz1.jpg'))
            self.medal.setIconSize(QSize(120, 120))
            self.label.setText('Молодец , ты неплохо знаешь машины , но не до конца')
        elif true_otw >= 8:
            self.medal.setIcon(QIcon('gold.png'))
            self.medal.setIconSize(QSize(120, 120))
