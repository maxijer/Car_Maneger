...
class Glaw_form(QWidget):  # Главная форма в ней находятся все кнопки
    def __init__(self):
        super().__init__()
        uic.loadUi('data/mashin.ui', self)
        self.korz.clicked.connect(self.korzin)
        self.play.clicked.connect(self.gamepad)
        self.volkswagen.clicked.connect(lambda: self.all_form("volkswagen"))  # привязываем функцию к
        # кнопкам
    ...
    def information(self):  # даём информацию
        global znach
    ...
    def sra(self):  # здесь происходит сравнение
        global sravnenie
    ...
    def korzin(self):  # добавление в корзину
        self.listWidget.clear()
        z = Clear(self)
        self.dobav_list(z)
        global korzina
...
class All_mashin(QWidget):  # класс для работы с машинами
    ...
    def add_korz(self):  # добавление в корзину
        global korzina
    ...
    def del_korzina(self):  # удаление из корзины
        global korzina
        model = self.model.text()
        con = sqlite3.connect('db/rabota.db')
    ...
    def delit(self):  # удаление автомобиля из базы
        model = self.model.text()
        con = sqlite3.connect('db/rabota.db')
    ...
    def sravnenie(self):  # добавление в сравнение
        global sravnenie
        global flag
        model = self.model.text()
        con = sqlite3.connect('db/rabota.db')
...
class Play(QWidget):  # класс для игры
    def __init__(self):
        super().__init__()
...
class Pokaz_trdea(QWidget):  # информация про цену машины после trade-in
